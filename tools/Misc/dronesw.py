...
class Drone:
    ...
    def update(self, lasers, drones):
        ...
        # Check if drone reached mission zone (escaped)
        mx, my, mw, mh = MISSION_ZONE
        if mx <= self.pos[0] <= mx+mw and my <= self.pos[1] <= my+mh:
            self.alive = False
...
def draw_thruster_glow(ax, drone):
    speed = np.linalg.norm(drone.velocity)
    if speed < 0.1:
        return
    glow_radius = 6 + 8 * speed / DRONE_MAX_SPEED
    glow_alpha = 0.3 + 0.4 * speed / DRONE_MAX_SPEED
    back_dir = np.array([-np.cos(drone.angle), -np.sin(drone.angle)])
    glow_center = drone.pos + back_dir * 7
    glow = Circle(glow_center, glow_radius, color='cyan', alpha=glow_alpha)
    ax.add_patch(glow)

def draw_thruster_trail(ax, drone):
    trail_length = 5
    for i in range(trail_length):
        factor = (trail_length - i) / trail_length
        pos = drone.pos - drone.velocity * DT * i * 2
        alpha = 0.15 * factor
        radius = 2 * factor
        trail_dot = Circle(pos, radius, color='cyan', alpha=alpha)
        ax.add_patch(trail_dot)

def draw_fog_shimmer(ax, frame):
    for fog in fog_patches:
        fog.set_alpha(0.15 + 0.05 * np.sin(frame * 0.1))
        ax.add_patch(fog)

def draw_lock_hud(ax, laser, drone):
    if not drone.alive:
        return
    drone_id = id(drone)
    lock_time = laser.lock_timers.get(drone_id, 0)
    progress = np.clip(lock_time / LASER_LOCK_TIME, 0, 1)
    if progress <= 0:
        return
    radius = 15
    start_angle = 90
    end_angle = 90 + 360 * progress
    wedge = Wedge(drone.pos, radius, start_angle, end_angle, color='red', alpha=0.6)
    ax.add_patch(wedge)
...
def update(frame):
    ...
    for drone in drones:
        if drone.alive:
            drone.draw(ax)
            draw_thruster_glow(ax, drone)
            draw_thruster_trail(ax, drone)
        else:
    ...
    # Mission zone
    x, y, w, h = MISSION_ZONE
    mission_patch = FancyBboxPatch((x,y), w, h, boxstyle="round,pad=0.5",
                                  linewidth=2, edgecolor='green', facecolor='none', alpha=0.7)
    ax.add_patch(mission_patch)
    ax.text(x + w/2, y + h/2, "MISSION ZONE", color='green', weight='bold',
            ha='center', va='center', fontsize=12, alpha=0.7)

    # HUD & Score
    alive_count = sum(d.alive for d in drones)
    ax.set_title(f'Drone Swarm vs Laser Troops — Drones Alive: {alive_count} | Escaped: {scoreboard["escaped"]} | Destroyed: {scoreboard["destroyed"]}', fontsize=14, weight='bold')
    ax.axis('off')
